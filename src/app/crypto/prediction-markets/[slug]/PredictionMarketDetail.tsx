...
export default function PredictionMarketDetail({ platform }: { platform: any }) {
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            <h1 className="text-4xl font-bold text-foreground mb-4">Prediction Market Not Found</h1>
            <Link href="/crypto/prediction-markets" className="text-purple-600 hover:text-purple-700">
  ...
  const stats = {
    total: platform.policies?.length || 0,
    critical: platform.policies?.filter((p: any) => p.impact === 'critical').length || 0,
    high: platform.policies?.filter((p: any) => p.impact === 'high').length || 0,
    good: platform.policies?.filter((p: any) => p.category === 'good').length || 0,
    bad: platform.policies?.filter((p: any) => p.category === 'bad').length || 0,
    common: platform.policies?.filter((p: any) => p.category === 'common').length || 0,
  };

  // Focus on critical user-impacting policies: funds, payouts, market resolution, account control
  const criticalPolicies = platform.policies?.filter((p: any) => {
    ...
    // CRITICAL: Market resolution and disputes (who decides outcomes?)
    const resolution = [
      'resolution', 'dispute', 'arbitration', 'oracle', 'outcome',
      'settle', 'settlement', 'invalid market', 'cancel market',
      'void', 'reverse outcome', 'incorrect resolution'
    ];

    // CRITICAL: Payout and withdrawal restrictions
    const payouts = [
      'payout', 'withdraw', 'withdrawal limit', 'lock', 'freeze funds',
      'withhold', 'delay', 'restrict withdrawal', 'hold funds',
      'cannot withdraw'
    ];
    ...
    const accountControl = [
      ...
      'disable account', 'revoke', 'without notice', 'discretion',
      'immediate termination'
    ];

    // CRITICAL: Fees and penalties
    const fees = [
      'fee', 'penalty', 'commission', 'withdrawal fee', 'inactivity fee',
      'hidden fee', 'additional charge'
    ];

    // CRITICAL: Liability and insurance
    const liability = [
    ...
    // CRITICAL: Data and privacy
    const privacy = [
      'share data', 'sell data', 'third party', 'government request',
      'law enforcement', 'disclose', 'mandatory arbitration',
      'class action waiver'
    ];

    const hasResolution = resolution.some(term => combined.includes(term));
    const hasPayouts = payouts.some(term => combined.includes(term));
    ...
    return hasResolution || hasPayouts || hasAccountControl || hasFees ||
           hasLiability || hasPrivacy || isCriticalImpact || p.category === 'good';
  }) || [];
  ...
  const rating = platform.overallRating ? ratingConfig[platform.overallRating as keyof typeof ratingConfig] : null;
  ...
  const sectionNames: Record<string, string> = {
    ...
    'AI/ML prohibitions': '🤖 AI/ML Restrictions',
    'intellectual property': '©️ Intellectual Property',
    'commercial use': '💼 Commercial Use',
    'monitoring/privacy': '👁️ Monitoring',
    other: '📋 Other'
  };

  const grouped = (platform.policies || []).reduce((acc: any, policy: any) => {
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <section className="bg-gradient-to-b from-gray-900 to-background py-8">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <Link href="/crypto/prediction-markets" className="text-purple-600 hover:text-purple-700 mb-4 inline-block text-sm">
            ← Back to Prediction Markets
          </Link>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            ...
            {platform.riskScore !== undefined && (
              <div className="bg-gray-900 border border-gray-700 rounded-lg p-3">
                <div className={`text-2xl font-bold ${
                  platform.riskScore <= 3 ? 'text-green-600' :
                  platform.riskScore <= 6 ? 'text-yellow-600' :
                  'text-red-600'
                }`}>
                  {platform.riskScore}/10
                </div>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {activeTab === 'overview' && (
            <div className="space-y-6">
              {platform.quickSummary && (
                <div className="bg-gray-900/30 border border-gray-800 rounded-xl p-6">
                  <h2 className="text-xl font-bold text-foreground mb-3">Summary</h2>
                  <p className="text-foreground/80 leading-relaxed">{platform.quickSummary}</p>
                  {platform.termsUrl && (
                    <a
                      href={platform.termsUrl}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-block mt-3 text-purple-600 hover:text-purple-700 text-sm"
                    >
                      Read Official Terms →
                    </a>
                  )}
              ...
              {platform.keyTakeaways && platform.keyTakeaways.length > 0 && (
                <div className="bg-purple-900/10 border border-purple-800 rounded-xl p-6">
                  <h2 className="text-xl font-bold text-foreground mb-4">Key Takeaways</h2>
                  <ul className="space-y-2">
                    {platform.keyTakeaways.map((takeaway: string, index: number) => (
                  ...
                  <div key={section} className="border border-gray-800 rounded-xl overflow-hidden bg-gray-900/30">
                    ...
                    >
                      <h3 className="text-lg font-bold text-foreground">
                        {sectionNames[section] || section} ({policies.length})
                      </h3>
                    ...
                    {isExpanded && (
                      <div className="border-t border-gray-800 p-4 space-y-3">
                        {policies.map((policy: any, pIndex: number) => {
                          ...
                          const color = policy.category === 'good' ? 'text-green-600' : policy.category === 'bad' ? 'text-red-600' : 'text-purple-600';
