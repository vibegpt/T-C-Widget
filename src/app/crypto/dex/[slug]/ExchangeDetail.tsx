...
export default function ExchangeDetail({ exchange }: { exchange: any }) {
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            <h1 className="text-4xl font-bold text-foreground mb-4">DEX Not Found</h1>
            <Link href="/crypto/dex" className="text-blue-600 hover:text-blue-700">
              ← Back to DEX List
            </Link>
          </div>
        </div>
  ...
  // Focus on critical user-impacting policies: funds control, smart contract risks, withdrawal restrictions
  const criticalPolicies = exchange.policies?.filter((p: any) => {
    ...
    // CRITICAL: Smart contract risks and exploits
    const smartContractRisks = [
      'smart contract', 'contract risk', 'bug', 'vulnerability', 'exploit',
      'hack', 'security breach', 'attack', 'malicious', 'reentrancy'
    ];

    // CRITICAL: Forced actions and liquidations
    const forcedActions = [
    ...
    // CRITICAL: Withdrawal and fund restrictions
    const withdrawalRestrictions = [
      'withdraw', 'withdrawal limit', 'lock', 'lock-up', 'vesting',
      'time lock', 'delay', 'restrict', 'freeze', 'hold funds'
    ];

    // CRITICAL: Fees and penalties
    const fees = [
      'fee', 'penalty', 'slippage', 'price impact', 'transaction cost',
    ...
    const liability = [
      ...
      'not responsible', 'insurance', 'compensation', 'protection fund',
      'indemnify', 'hold harmless'
    ];

    // CRITICAL: Protocol governance changes
    const governance = [
      'change terms', 'modify protocol', 'upgrade', 'governance',
      'vote', 'proposal', 'admin key', 'multisig'
    ];

    const hasSmartContractRisk = smartContractRisks.some(term => combined.includes(term));
    const hasForcedActions = forcedActions.some(term => combined.includes(term));
    const hasWithdrawalRestriction = withdrawalRestrictions.some(term => combined.includes(term));
    const hasFees = fees.some(term => combined.includes(term));
    const hasLiability = liability.some(term => combined.includes(term));
    const hasGovernance = governance.some(term => combined.includes(term));

    // Also include critical and high impact policies
    const isCriticalImpact = p.impact === 'critical' || p.impact === 'high';

    return hasSmartContractRisk || hasForcedActions || hasWithdrawalRestriction ||
           hasFees || hasLiability || hasGovernance || isCriticalImpact || p.category === 'good';
  }) || [];
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <section className="bg-gradient-to-b from-gray-900 to-background py-8">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <Link href="/crypto/dex" className="text-blue-600 hover:text-blue-700 mb-4 inline-block text-sm">
            ← Back to DEXs
          </Link>
            ...
            <div className="space-y-4">
              {criticalPolicies.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <h3 className="text-xl font-bold text-foreground mb-2">No Critical Risks Found</h3>
                  <p className="text-foreground/60">This platform has no policies marked as critical or high impact.</p>
                </div>
              ) : (
                <>
                  <div className="bg-red-900/20 border border-red-800 rounded-xl p-4 mb-6">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <h3 className="font-bold text-red-600 mb-1">Critical & High Risk Policies</h3>
                        <p className="text-sm text-foreground/70">
                          These policies have significant impact on your funds or rights. Review carefully.
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  {criticalPolicies.map((policy: any, index: number) => (
                    <div
                      key={index}
                      className="border border-red-800 bg-red-900/10 rounded-xl overflow-hidden"
                    >
                      <button
                        onClick={() => togglePolicy(index)}
                        className="w-full p-4 flex items-center justify-between hover:bg-red-900/20 transition-colors"
                      >
                        <div className="flex items-center gap-3 flex-1 text-left">
                          <span className="text-xl">⚠️</span>
                          <div>
                            <h4 className="font-bold text-red-600">{policy.title}</h4>
                      ...
                      {expandedPolicies.has(index) && policy.details && (
                        <div className="px-4 pb-4 border-t border-red-800/50">
